"""
this file is an initial example of usage of the library.

In this example a simple multi objective custom environment is specified by the user and the environment is generated by the library

The idea is that the LMORL.Environment object has a method that returns a scalar reward 
(obtained, in case of MultiObjective reward array, by multiplying the MO reward array by its weights (that can be Non Archimedean))
...
# end of for development purposes only

from LMORL.Environment import Environment as Env
from LMORL.spaces import Box, Dict, Discrete, MultiBinary

# this latter import is outside of the scope of our library
...
if __name__ == "__main__":
    ...
    render_mode = "human"
    env = BreakableBottles(size=5, prob_drop=0.1, render_mode=render_mode)
    ...
    while True:
        env.render()
        obs, reward, done, info, not_sium = env.step(env.action_space.sample())
        print(reward)
        if done:
            break